			...
			<div class="row">
	                ...
	                <?php
	                	...
	                	if ($_SESSION["tipo"]=="root") {
	                		$sql_act="SELECT * FROM loteria WHERE activo='1' AND ganar='3' AND (fecha BETWEEN '".$desde."' AND '".$hasta."')";

                            $sql_t_apostado="SELECT SUM(monto) AS t_apostado FROM loteria WHERE activo='1' AND ganar='3' AND (fecha BETWEEN '".$desde."' AND '".$hasta."')";
	                	...
	                	elseif ($_SESSION["tipo"]=="chance") {
                            ...
                            $sql_t_apostado="SELECT SUM(monto) AS t_apostado FROM loteria WHERE activo='1' AND ganar='3' AND agencia='".$_SESSION["agencia"]."' AND (fecha BETWEEN '".$desde."' AND '".$hasta."')";
                            $rs_t_apostado=mysqli_query($mysqli, $sql_t_apostado) or die(mysqli_error());
                            $row_t_apostado=mysqli_fetch_array($rs_t_apostado);
	                	}
	                    ...
	                    while ($row_act=mysqli_fetch_array($rs_act)) {
	                    	  ...
	                    	  $sql_usr="SELECT nombre, apellido, tipo FROM usuarios WHERE cedula='".$row_act["cedula"]."'";
	                    		...
	                    		echo"<tr>";
	                    			echo"<td>";
	                    				echo "<a href='con_activo_l.php?codigo=".$row_act["codigo"]."&desde=".$desde."&hasta=".$hasta."'>".$row_act["codigo"]."</a>";
	                    			echo"</td>";
	                    			echo"<td>";
